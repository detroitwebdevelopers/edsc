<?php
/*
Plugin Name: Auto Login
Description: Automatically tries to login to admin
Version: 1.0
Author: cloudaccess.net
*/

if (basename($_SERVER['PHP_SELF']) == basename (__FILE__)) {
	die('Sorry, but you cannot access this page directly.');
}

if(!session_id()) {
    session_start();
}

add_action('init', 'caStartSession', 1);
function caStartSession() {
    if(!session_id()) {
        session_start();
    }
}

add_filter( 'login_errors', 'ca_custom_login_error' );
/*
* @desc    Filter Wordpress admin error message
* @param str $error Error message to be changed.
*/
function ca_custom_login_error( $error ){
    return $error;
}

// function to auto login into admin
function ca_auto_login() {	
	global $error;

	if (isset($_SESSION['CAERRORS'])) {
		$error = $_SESSION['CAERRORS'];
		unset($_SESSION['CAERRORS']);
		return false;
	}

	if (is_admin() && intval($_GET['cadebug'])) {
		echo '<pre>';
		var_dump($_SESSION);
		var_dump($_COOKIE);
		echo '</pre>';
	}

	// only check token if user are not logged in wp-admin
	if ( is_admin() && (isset($_GET['catoken']) && $_GET['catoken'] != '') && intval($_GET['ccp']) && !is_user_logged_in() )
	{
		// alphanum filter
		$token = preg_replace('/[^a-zA-Z0-9_]/', '', $_GET['catoken']);		

		// use curl for request
		$response = new stdclass;
		$ch = curl_init();
		curl_setopt($ch,CURLOPT_URL, 'http://sso.cloudaccess.net/');
		curl_setopt($ch,CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS, 'token='.$token);
		$response->body = curl_exec($ch);
		$response->code = curl_getinfo($ch, CURLINFO_HTTP_CODE);
		curl_close($ch);
		
		if ($response->code != 200) {
			// add error msg
			$_SESSION['CAERRORS'] = '<strong>Cloudaccess auhtentication:</strong> service not available';
			return false;
		} else {
			if (empty($response->body)) {
				$_SESSION['CAERRORS'] = '<strong>Cloudaccess auhtentication:</strong> empty response frmo service';
				return false;
			} else {
				$credentials = json_decode($response->body, true);

				if (!$credentials['success']) {
					// add error msg
					$_SESSION['CAERRORS'] = '<strong>Cloudaccess auhtentication:</strong> '.(string)$credentials['error'];
					return false;
				} else {
					$creds = array();
					$creds['user_login'] = $credentials['data']['username'];
					$creds['user_password'] = addslashes($credentials['data']['password']);
					$creds['remember'] = false;

					wp_clear_auth_cookie();
					$secure_cookie = '';
					if ( $user = get_user_by('login', $creds['user_login']) ) {
						if ( get_user_option('use_ssl', $user->ID) ) {
							$secure_cookie = true;
							force_ssl_admin(true);
						}
					}
					$user = wp_signon( $creds, $secure_cookie );
					if( is_wp_error( $user ) )
					{
						$error = $user->get_error_message();
					}
					else 
					{			    
						$userID = $user->ID;
						wp_set_current_user( $userID );
						wp_set_auth_cookie($userID);
						
					    if ( is_user_logged_in() ) 
						{ 
						    wp_redirect( home_url('wp-admin/') );exit();//dont remove this exit, else admin will not be logged in
						}
					}
				}
			}
		}
    }
}

add_action( 'after_setup_theme', 'ca_auto_login', 1 );
